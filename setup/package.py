...
version_docker = version.replace('+','-')
...
def _prepare_testing(o):
    if not o.no_tarball:
        ...
        subprocess.call(["docker", "build", "-t", "odoo-%s-src-nightly-tests" % version_docker, "."],
    ...
    if not o.no_debian:
        ...
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_debian")],
                        cwd=os.path.join(o.odoo_dir))
        subprocess.call(["docker", "build", "-t", "odoo-%s-debian-nightly-tests" % version_docker, "."],
    ...
    if not o.no_rpm:
        ...
        subprocess.call(["cp", "package.dffedora", os.path.join(o.build_dir, "docker_fedora", "Dockerfile")],
                        cwd=os.path.join(o.odoo_dir, "setup"))
        subprocess.call(["docker", "build", "-t", "odoo-%s-fedora-nightly-tests" % version_docker, "."],
                        cwd=os.path.join(o.build_dir, "docker_fedora"))

def test_tgz(o):
    with docker('odoo-%s-src-nightly-tests' % version_docker, o.build_dir, o.pub) as wheezy:
...
def test_deb(o):
    with docker('odoo-%s-debian-nightly-tests' % version_docker, o.build_dir, o.pub) as wheezy:
        wheezy.release = '*.deb'
        wheezy.system("service postgresql start")
        wheezy.system('su postgres -s /bin/bash -c "createdb mycompany"')
...
def test_rpm(o):
    with docker('odoo-%s-fedora-nightly-tests' % version_docker, o.build_dir, o.pub) as fedora24:
